...
using irunsaapp.Data.GoogleMap;
    ...
    public partial class GoogleMap : MudComponentBase, IAsyncDisposable
    {
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender)
            {
                 ...
                 await _jsRuntime.InvokeVoidAsync("BlazorStore.createMap", Element, Reference, UniqueID, ApiKey, Zoom, Center,
                     data.Select(m => new { Title = m.Title, Label = m.Label, Position = m.Position }));
            }
            else
            {
                await _jsRuntime.InvokeVoidAsync("BlazorStore.updateMap", UniqueID, Zoom, Center,
                             data.Select(m => new { Title = m.Title, Label = m.Label, Position = m.Position }));
            }
        }
        ...
        public async Task MoveToCenter()
        {
            ...
            await _jsRuntime.InvokeVoidAsync("BlazorStore.updateMap", UniqueID, Zoom, Center,
                            data.Select(m => new { Title = m.Title, Label = m.Label, Position = m.Position }));
            //await _jsRuntime.InvokeVoidAsync("BlazorStore.moveToCenter", UniqueID, Center);
        }
        public ValueTask DisposeAsync()
        {
            return _jsRuntime.InvokeVoidAsync("BlazorStore.destroyMap", UniqueID);
        }
